function get-userInputs {
  ...
  if (($inputSubnetMask -gt 0) -and ($inputSubnetMask -le 32)) {
...
function validateMask {
  ...
  if (($subnetMaskX.count -gt 1) -and (validateIP -ip $subnetMask)) {
    ...
    foreach ($octet in $subnetMaskX) {
      #if ($octet -gt 255) {
      #  write-host "ERROR: Subnet Octet needs to be < 255"
      #  return $false
      #}
...
function calculate-vlsm {
  ...
  # Sort Subnets by Host Required
  $global:db['subNets'] = $global:db['subNets'] | Sort-Object { $_.hosts_req } -Descending
...
function csv-printer {
  $file = "output.csv"
  $header = @("net_common_name", "hosts_req", "net_add", "sub_start", "sub_end", "sub_broad",
  ...
  foreach ($subnet in $global:db['subNets']) {
    $newRow = New-Object PsObject -Property @{
      net_common_name=($subnet['net_common_name']);
      hosts_req=($subnet['hosts_req']);
